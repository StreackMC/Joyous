...
public class entry extends JavaPlugin {
  ...
  public static final class Models {
    ...
    static {// 在这里统一注册模块，下方自动遍历处理。
      ...
      addModel(new JMenuMain());
      addModel(new MailsMain());
      addModel(new RestarterMain());
    }
  }
  ...
  public void onEnable() {
    ...
    i18n.init(Joyous.dataPath);
    if (Files.exists(Joyous.dataPath.toPath().resolve("language.new.yml"))) {
      jlogger.warn(i18n.tr("system.i18n.update"), Joyous.dataPath.toPath().resolve("language.new.yml").toAbsolutePath());
    }
    try {
      Joyous.confDefault = new SConfig(Joyous.getResourceAsFile("/config.yml"), "yml");
    } catch (Exception e1) {
      try {
        jlogger.warn("无法载入内联配置文件，正使用空配置替代……：", e1);
        Joyous.confDefault = new SConfig("","yml","joyous");
      } catch (Exception e2) {
        // 真到这了也没必要继续运行了
        jlogger.severe("无法载入内联配置文件，也无法使用空配置替代：", e2);
    ...
    try {
      ...
      try {
        jlogger.warn("无法载入构建信息文件，正使用空配置替代……：", e1);
        Joyous.confBuild = new SConfig("","yml","joyous");
      } catch (Exception e2) {
        // 真到这了也没必要继续运行了
        jlogger.severe("无法载入构建信息文件，也无法使用空配置替代：", e2);
        Joyous.pluginManager.disablePlugin(this);
      }
    }
    ...
    try {
      ...
      jlogger.severe(e.getLocalizedMessage(), e);
      Joyous.pluginManager.disablePlugin(this);
      return;
    }
    ...
    Models.models.forEach((name, model) -> {
      try {
        if (!Joyous.conf.getBoolean(name + ".enabled", true)) {
          jlogger.debug("由于配置文件，已禁用 " + name);
          return;
        }
        jlogger.info("正在启用 " + name);
        model.onEnable();
      } catch (IgnoredException e1) {
        jlogger.debug("中断" + e1.getLocalizedMessage() + "\n", e1.getStackTrace().toString());
      } catch (Exception e2) {
        jlogger.severe("启用 %s 时发生错误：%s", name, e2.getLocalizedMessage(), e2);
      }
    });
    jlogger.info("已尝试启用全部子模块");
  }

  @Override
  public void onDisable() {
    jlogger.info("正在禁用子模块...");
    Models.models.forEach((name, model) -> {
      try {
        jlogger.info("正在禁用 " + name);
        model.onDisable();
      } catch (Exception e) {
        jlogger.warn("禁用 %s 时发生错误：%s", name, e.getLocalizedMessage(), e);
      } finally {
        jlogger.info("已尝试禁用 " + name);
      }
    });
    jlogger.info("已尝试禁用全部子模块");
  }

  private void CheckConfigUpdate() {
    jlogger.info("正在检查配置文件：" + new File(Joyous.dataPath, "config.yml").getPath());
    int diff = Joyous.getConfigVerisonDiff();
    if (diff == 0) {
      jlogger.info("配置文件版本正常，无需更新。");
      return;
    }
    if (diff > 0) {
  ...
  private void CheckDependencies() throws RuntimeException {
    ...
    if (Joyous.isDebugMode()) {
      jlogger.debug("检测到StreackLib，版本：" + StreackLib.ENV.buildConf.getString("version"));
      jlogger.warn("你正在StreackLib中使用调试模式并已继承到Joyous中，因此会收到更多信息。");
    }

    /* 检测 PlaceholderAPI */
    if (!Joyous.pluginManager.isPluginEnabled("PlaceholderAPI")) {
      jlogger.info("未检测到PlaceholderAPI，相关功能将无法使用。");
      Joyous.PlaceholderService = new PHAPI(false);
    } else {
      Joyous.PHAPI_available = true;
      Joyous.PlaceholderService = new PHAPI(true);
      jlogger.debug("检测到PlaceholderAPI，版本：" + Joyous.pluginManager.getPlugin("PlaceholderAPI").getPluginMeta().getVersion());
    }
  }
